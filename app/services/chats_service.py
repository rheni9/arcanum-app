...
def get_chat_by_slug(slug: str) -> Chat | None:
    ...
    try:
        ...
        if not chat:
            logger.warning("[CHATS|SERVICE] No chat found with slug '%s'.",
                           slug)
            return None
        return chat
    except SQLAlchemyError as exc:
        logger.error("[CHATS|SERVICE] Failed to retrieve chat by slug: %s",
                     exc)
        raise
...
def slug_exists(slug: str) -> bool:
    ...
    try:
        ...
        logger.error("[CHATS|SERVICE] Failed to check slug existence: %s",
                     exc)
        raise
...
def get_global_stats() -> dict:
    ...
    try:
        ...
        logger.error("[CHATS|SERVICE] Failed to retrieve global stats: %s",
                     exc)
        raise
